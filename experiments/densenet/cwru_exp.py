...
# --- CONFIGURAÇÕES ---
BASE_DRIVE = os.getcwd() # Ou defina o caminho absoluto
DATASET_FINAL = os.path.join(BASE_DRIVE, "dataset_final")
IMG_SIZE = 224
BATCH_SIZE = 32
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
OUTPUT_FILE = "cwru_results.txt"
...
def load_dataset_data(ds_name, root_dir):
    ...
    paths, labels = [], []
    # Mapa local de classes para garantir consistência no pré-treino
    # Para VibNet (Source), não importa tanto a ordem exata, desde que seja consistente
    label_map = {} 
    ...
    for root, dirs, files in os.walk(ds_path):
        for file in files:
            if file.endswith(".png"):
                # Estrutura: Dataset/Condition/Class/img.png
...
def train_vibnet_source(target_dataset):
    """
    Treina o modelo fonte excluindo o dataset alvo.
    Se alvo for CWRU_12k, usa CWRU_48k no treino (e vice versa).
    """
    print(f"\n>>> Treinando VibNet Source (Excluindo {target_dataset})...")
    
    # Lista de todos os datasets possíveis
    all_datasets = ["CWRU_12k", "CWRU_48k", "HUST", "UORED", "PU"]
    
    # Remove o alvo da lista de fontes
    ...
    for epoch in range(5): # 5 épocas rápidas para pré-treino
        model.train()
        total_loss = 0
    ...
    w_path = f"vibnet_source_no_{target_dataset}.pth"
    torch.save(model.state_dict(), w_path)
    return w_path

def run_cwru_experiment(ds_name):
    print(f"\n{'#'*50}")
    print(f"INICIANDO EXPERIMENTO: {ds_name}")
    print(f"{'#'*50}")

    ds_root = os.path.join(DATASET_FINAL, ds_name)
    if not os.path.exists(ds_root):
        print(f"Dataset {ds_name} não encontrado. Pulando.")
        return

    # 1. Pesos Fonte
    weights_path = f"vibnet_source_no_{ds_name}.pth"
    if not os.path.exists(weights_path):
        weights_path = train_vibnet_source(ds_name)
    else:
        print(f"Pesos pré-treinados encontrados: {weights_path}")

    # 2. Detectar Estrutura
    # No CWRU temos pastas como 'Normal_Load_0', 'Severity_007', etc.
    conditions = sorted([d for d in os.listdir(ds_root) if os.path.isdir(os.path.join(ds_root, d))])
    
    # Mapear classes (Normal, Inner, Outer, Ball)
    # Pega da primeira pasta válida para criar o mapa
    first_cond = os.path.join(ds_root, conditions[0])
    classes = sorted([d for d in os.listdir(first_cond) if os.path.isdir(os.path.join(first_cond, d))])
    cls_map = {c: i for i, c in enumerate(classes)}
    
    print(f"Condições: {conditions}")
    print(f"Classes: {cls_map}")

    strategies = ["Scratch", "ImageNet", "VibNet"]
    results = []

    # 3. LODO-CV Loop (Leave-One-Condition-Out)
    for test_cond in conditions:
        train_conds = [c for c in conditions if c != test_cond]
        
        # Opcional: Para poupar tempo, se houver MUITAS condições, 
        # teste apenas algumas ou agrupe. Mas CWRU tem poucas (~5-8).
        ...
        # Carrega Teste
        c_path = os.path.join(ds_root, test_cond)
        for cls in classes:
            if cls not in cls_map: continue
            p = os.path.join(c_path, cls)
            if os.path.exists(p):
        ...
        # Split de Validação (20% do treino)
...
if __name__ == "__main__":
    ...
    # 1. Roda para CWRU 12k
    res_12k = run_cwru_experiment("CWRU_12k")
    if res_12k: all_results.extend(res_12k)

    # 2. Roda para CWRU 48k
    res_48k = run_cwru_experiment("CWRU_48k")
    if res_48k: all_results.extend(res_48k)

    # 3. Consolidação
    if all_results:
        df = pd.DataFrame(all_results)
        print("\n\n" + "="*50)
        print("RELATÓRIO FINAL CWRU (12k + 48k)")
        print("="*50)
        print(df.to_string())
