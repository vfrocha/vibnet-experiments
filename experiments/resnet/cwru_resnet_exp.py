...
OUTPUT_FILE = "cwru_resnet_results.txt" # Nome do arquivo de log alterado
...
def train_vibnet_source_resnet(target_dataset):
    """
    Versão adaptada para treinar uma RESNET-18 como modelo fonte (Source).
    """
    ...
    # ResNet usa 'fc' em vez de 'classifier'
    ...
    # Salva com nome específico para ResNet
    w_path = f"vibnet_resnet_source_no_{target_dataset}.pth"
    torch.save(model.state_dict(), w_path)
    return w_path

def run_cwru_resnet_experiment(ds_name):
    print(f"\n{'#'*50}")
    print(f"INICIANDO EXPERIMENTO RESNET: {ds_name}")
    print(f"{'#'*50}")
    ...
    # 1. Pesos Fonte (Específicos para ResNet)
    weights_path = f"vibnet_resnet_source_no_{ds_name}.pth"
    if not os.path.exists(weights_path):
        weights_path = train_vibnet_source_resnet(ds_name)
    else:
        print(f"Pesos VibNet (ResNet) encontrados: {weights_path}")
    ...
    for test_cond in conditions:
        ...
        for strat in strategies:
            ...
            n_feat = model.fc.in_features # ResNet usa 'fc'
            
            if strat == "ImageNet":
                temp = models.resnet18(weights='IMAGENET1K_V1')
                # Carrega tudo menos a camada final 'fc'
                ...
                curr = model.state_dict()
                # Filtra a camada 'fc' pois o source tem numero de classes diferente
                pt_dict = {k: v for k, v in state.items() if k in curr and 'fc' not in k}
                curr.update(pt_dict)
                model.load_state_dict(curr)

            # Nova camada final
            model.fc = nn.Linear(n_feat, len(cls_map))
            model = model.to(DEVICE)
...
if __name__ == "__main__":
    ...
    # Roda ResNet para CWRU 12k
    res_12k = run_cwru_resnet_experiment("CWRU_12k")
    if res_12k: all_results.extend(res_12k)

    # Roda ResNet para CWRU 48k
    res_48k = run_cwru_resnet_experiment("CWRU_48k")
    if res_48k: all_results.extend(res_48k)

    if all_results:
        df = pd.DataFrame(all_results)
        print("\n\n" + "="*50)
        print("RELATÓRIO FINAL CWRU - RESNET18")
        print("="*50)
        print(df.to_string())
