...
OUTPUT_FILE = "hust_resnet_results.txt"
...
# --- PRÉ-TREINO (SOURCE) ---
def train_vibnet_source_resnet(target_dataset="HUST"):
    """Treina ResNet-18 em todas as bases MENOS a target (HUST)."""
    ...
    model.fc = nn.Linear(model.fc.in_features, offset) # Ajusta saída para total de classes source
...
# --- CONFIGURAÇÃO HUST ---
# Classes e Condições específicas do HUST
HUST_CLASS_MAP = {
...
def get_hust_splits(root_dir, train_conds, test_conds):
    ...
    for cond in os.listdir(root_dir):
        if cond not in HUST_CONDS: continue
...
def run_hust_resnet_experiment():
    print(f"\n{'#'*50}")
    print(f"INICIANDO EXPERIMENTO RESNET: HUST")
    print(f"{'#'*50}")

    hust_root = os.path.join(DATASET_FINAL, "HUST")
    if not os.path.exists(hust_root):
        print("Dataset HUST não encontrado.")
        return

    # 1. Garante Pesos VibNet (ResNet)
    weights_path = "vibnet_resnet_source_no_HUST.pth"
    if not os.path.exists(weights_path):
        weights_path = train_vibnet_source_resnet("HUST")
    else:
    ...
    # 2. Loop LODO-CV (Leave-One-Load-Out)
    for test_cond in HUST_CONDS:
        ...
        for strat in strategies:
            ...
            if strat == "ImageNet":
                ...
                curr = model.state_dict()
                # Remove fc weights (pois source tinha N classes, HUST tem 7)
                pt_dict = {k: v for k, v in state.items() if k in curr and 'fc' not in k}
                curr.update(pt_dict)
                model.load_state_dict(curr)

            # Nova camada final para 7 classes do HUST
            model.fc = nn.Linear(n_feat, len(HUST_CLASS_MAP))
            model = model.to(DEVICE)
    ...
    print("\n\n" + "="*50)
    print("RELATÓRIO FINAL HUST - RESNET18")
    print("="*50)
    print(df.to_string())
